...
async function transactionValidator(tx: TransactionInterface) {
  ...
  if (tx.inputs) {
    ...
    const inputValidator = async (input: { outpoint: any; sig: any }) => {
      // validate outpoints
      const itx = await Transaction.findOne({
        objectId: input.outpoint.txid,
      }).exec();
      if (!itx) {
        throw new Error(
          "Did not find previous transaction with id: " + input.outpoint.txid
        );
      }
      logger.info(`Found previous transaction: ${JSON.stringify(itx)}`);
      if (input.outpoint.index >= itx.outputs.length) {
        throw new Error(
          `Outpoint index out of bound: ${input.outpoint.index}/${
            itx.outputs.length - 1
          }`
        );
      }
      // validate signatures
